...
namespace toMap
{
    public static class Function1
    {
        ...
        public static async Task Run([EventGridTrigger]EventGridEvent eventGridEvent, ILogger log)
        {
            ...
            if (message["data"]["modelId"].ToString() == "dtmi:itri:cms:perceptor;1")
            {   //Set the ID of the room to be updated in your map. 
                ...
                foreach (var operation in message["data"]["patch"])
                {
                    ...
                    log.LogInformation("Before feature id " + featureID);
                    if (operation["op"].ToString() == "add" && operation["path"].ToString() == "/CrowedAlarm" && featureID.Length > 4)
                    {   //Update the maps feature stateset

                        log.LogInformation("crowed alram " + operation["value"].Value<int>());
                        ...
                        //DateTime TW_DataTime = currentDate.AddHours(8);
                        // Get Tokyo Standard Time zone
                        //TimeZoneInfo tst = TimeZoneInfo.FindSystemTimeZoneById("Pacific Standard Time");
                        //DateTime tstTime = TimeZoneInfo.ConvertTime(currentDate, TimeZoneInfo.Local, tst);

                        //log.LogInformation("utc now " + TW_DataTime.ToString());
                        ...
                        var postcontent = new JObject(new JProperty("States", new JArray(
                            ...
                            //new JProperty("eventTimestamp", DateTime.Now.ToLocalTime().ToString("S"))))));
                            new JProperty("eventTimestamp", DateTime.Now.AddHours(8).ToString("s"))))));
                        ...
                        log.LogInformation("state "+ statesetID);
                        log.LogInformation("feature id "+featureID);
                        log.LogInformation("subscription-key "+ subscriptionKey);
                        log.LogInformation("post content " + postcontent.ToString());
                        log.LogInformation("url " + $"https://atlas.microsoft.com/featureState/state?api-version=1.0&statesetID={statesetID}&featureID={featureID}&subscription-key={subscriptionKey}");
