...
export default {
  ...
  data() {
    return {
      ...
      toolbarTooltipSettingRPC: 'Setting Percept',
      toolbarTwinType: '',
      toolbarTwinId: '',
      toolbarParentId: '',
  ...
  methods: {
    ...
    tooltipTitle(action) {
      if (action === 'add') {
        return this.toolbarTwinType === 'floor' ? 'Add Room Twin' : 'Add Device Twin';
      }
      return action === 'map' ? 'Open Indoor Map' : 'Delete Device Twin';
    },
      ...
      for (const k in obj) {
        if (obj[k] instanceof Object) {
          ...
          if (obj[k].startsWith(searchString)) {
            ...
            if (searchString === 'percept-') {
              const tmp = obj[k].split('-');
              this.twinEndName[parseInt(tmp[3].replace('00', ''), 10) - 1] = obj[k];
            } else {
              this.twinEndName[parseInt(obj[k].replace('MPR', ''), 10) - 1] = obj[k];
            }
          }
        }
      }
    ...
    addRPC(toolbarTwinType) {
      ...
      if (toolbarTwinType === 'room') {
        this.getTwinCount(this.$store.getters.treeJSON, 'percept-');
        const addRpcName = `percept-dt-00${this.twinEndName.indexOf('') + 3}`;
        Promise.all([
          this.addRpcTwinXY(addRpcName, 0, 0),
        ]).then(() => {
          this.addTwinRelation(this.toolbarTwinId, `${this.toolbarTwinId}_to_${addRpcName}`, addRpcName, 'devices');
          const newNode = {
            name: addRpcName,
        ...
        Promise.all([
          ...
          this.addTwinRelation('F19', `F19_to_${addRpcName}`, addRpcName, 'rooms');
          const newNode = {
            name: addRpcName,
            parentId: 'F19',
            relationshipId: `F19_to_${addRpcName}`,
            type: 'room',
            children: [
            ],
          };
          this.insertNodeIntoTree(this.$store.getters.treeJSON, '19 樓', newNode);
        }).catch((res) => {
          console.error(res);
          this.isLoading = false;
        });
