...
export default {
  ...
  methods: {
    ...
    addRPC() {
      const addRpcName = `${this.rpcPrefixName}${this.twinEndName.indexOf('') + 1}`;
      console.log(`addRpcName: ${addRpcName}, featureId: ${this.featureId}`);
      Promise.all([
        this.addRpcTwin(addRpcName, this.featureId),
      ]).then(() => {
        this.addRoomRpcRelation(this.roomName,
          `${this.roomName}_to_${addRpcName}`, addRpcName, 'devices');
        const newNode = {
          name: addRpcName,
